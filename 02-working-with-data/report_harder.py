...
def main():
    ...
    acting_with_judd = we_played_with_judd(movie_library)
    flattened_acting_with_judd_list = flatten_acting_with_judd(acting_with_judd)
    movie_library = playing_with_judd_pays(movie_library, flattened_acting_with_judd_list)
    ally_sheedy_dict = get_ally_sheedy_movies(movie_library)
    get_best_ally_sheedy_movie(ally_sheedy_dict)
...
def we_played_with_judd(movie_library):
    acting_with_judd = []
    for movie in movie_library:
...
def flatten_acting_with_judd(acting_with_judd):
...
def get_ally_sheedy_movies(movie_library):
    ally_sheedy_dict = []
    for movie in movie_library:
        for name in movie.get('actors'):
                if name == 'Ally Sheedy':
...
def get_best_ally_sheedy_movie(ally_sheedy_dict):
    count, best = 0, ''
    for movie in ally_sheedy_dict:
        rating = movie.get('rating')
        if rating > count:
            count, best = rating, movie.get('title')
