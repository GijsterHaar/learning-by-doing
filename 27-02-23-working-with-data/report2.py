...
def main():
    ...
    average_rating(movie_library, count=0)
    best_movie(movie_library, highest=0)
    worst_movie(movie_library, lowest=11)

def total_movies(movie_library):
    print(f'\nThere are {len(movie_library)} movies in this folder')

def average_rating(movie_library, count):
    for movie in movie_library:
        count += movie.get('rating')
    print(f'\nThe average rating is {count/len(movie_library)}')

def best_movie(movie_library, highest):
    for movie in movie_library:
...
def worst_movie(movie_library, lowest):
    for movie in movie_library:
